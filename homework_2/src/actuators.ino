//hw2 by Barbova Tatiana în cadrul bootcampului IoT
//configure PIR	
...
void loop() {
  ...
  if ((statePIR == 1) && (stateLight == 0)) { //daca este miscare si este intuneric
    if (stateRelay == 0) {
      turnRelayOn(); //sa aprindem numai daca nu este aprins
    }
  }
  if (stateRelay == 1) {
    turnRelayOff(); //daca este aprins si nu sunt indeplenite conditii, stingem
  }
}